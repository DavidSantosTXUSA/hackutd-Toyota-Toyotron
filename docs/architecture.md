...
**Challenge:** Traditional e-commerce is reactive (user searches → results). This app requires **proactive guidance** where the agent drives the conversation based on user preferences.

**Solution Components:**

1. **Preference Loader** - Extract user quiz answers and store in `preferencesAtom`
2. **Agent Orchestrator** - Format preferences + car inventory as context for Nemotron
3. **Response Processor** - Parse agent suggestions to extract car IDs and reasoning
4. **UI Renderer** - Display agent suggestions as interactive car cards with "why" explanation
...
Send to POST /api/agent/chat with:
  ...
  - Available cars (from Supabase)
  - Chat history (from atom)
    ↓
Nemotron responds with suggestions + reasoning
    ↓
Parse response for car IDs
    ↓
Fetch full car data from Supabase
    ↓
Render ChatMessage with:
  - Agent's explanation
...
- Parse Nemotron response for structured car suggestions (carId, reasoning)
- Cache chat history in `chatAtom` to avoid duplicate API calls
- Include available cars in API context so agent can reference actual inventory
- Fetch car details after agent response, don't assume agent has full specs
